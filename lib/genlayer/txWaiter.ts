  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    try {
      ...
      if (receipt) {
        ...
        console.log("[Credence] TX poll status:", status);
        if (status === "accepted" || status === "finalized") return status;
        if (status === "cancelled" || status === "undetermined") throw new Error(`Transaction ${status}`);
      }
    } catch (e) {
      if (e instanceof Error && (e.message.includes("cancelled") || e.message.includes("undetermined"))) throw e;
    }
