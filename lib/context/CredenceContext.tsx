...
import {
  SEED_BORROWERS, SEED_POOLS, SEED_POLICIES, SEED_REVIEWS, SEED_LOANS, SEED_DEFAULTS, SEED_APPEALS, SEED_STATS,
} from "@/lib/credence/seed";
...
export function CredenceProvider({ children }: { children: React.ReactNode }) {
  const [borrowers, setBorrowers] = useState<Borrower[]>(SEED_BORROWERS);
  const [pools, setPools] = useState<LenderPool[]>(SEED_POOLS);
  const [policies, setPolicies] = useState<RiskPolicy[]>(SEED_POLICIES);
  const [reviews, setReviews] = useState<CreditReview[]>(SEED_REVIEWS);
  const [loans, setLoans] = useState<Loan[]>(SEED_LOANS);
  const [defaults, setDefaults] = useState<DefaultReview[]>(SEED_DEFAULTS);
  const [appeals, setAppeals] = useState<CreditAppeal[]>(SEED_APPEALS);
  ...
  return (
    <CredenceContext.Provider value={{
      ...
      addLoan, addDefault, updateDefault, addAppeal, updateAppeal,
    }}>
      {children}
