...
export default function LoanDeskPage() {
  ...
  const [form, setForm] = useState({
    ...
    loanPurpose: "WORKING_CAPITAL", preferredCollateral: "325", repaymentPlan: "Single repayment at maturity",
  });
  ...
  async function handleSubmit() {
    if (!address || !form.poolId || !form.reviewId || !verdict) return;
    ...
    const loan: Loan = {
      ...
      collateral: `${Math.ceil(Number(form.amount) * ratio / 100)} USDC`,
      collateralRatio: ratio,
      durationDays: form.durationDays,
      status: "ACTIVE",
      createdAt: new Date().toISOString(),
      dueAt: addDays(new Date(), form.durationDays).toISOString(),
    };
    addLoan(loan);
    setSubmitted(true);
  }
    ...
    <AppShell title="Loan Desk">
      <div className="p-6 space-y-5 max-w-3xl mx-auto">
        ...
        <div className="panel p-3 border-[rgba(242,169,59,0.3)] bg-[rgba(242,169,59,0.04)] text-[12px] text-ink flex items-start gap-2">
          ...
          Your requested collateral ratio may be adjusted by GenLayer review. The lender&apos;s risk policy controls the final acceptable band. Reduced collateral is not guaranteed.
        </div>
        ...
        {!submitted ? (
          <div className="panel p-5 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-[10px] font-financial uppercase tracking-widest text-muted-ink block mb-1">Credit Review (approved)</label>
              ...
              <div>
                <label className="text-[10px] font-financial uppercase tracking-widest text-muted-ink block mb-1">Preferred Collateral (USDC)</label>
                <input type="number" value={form.preferredCollateral}
                  onChange={(e) => setForm((f) => ({ ...f, preferredCollateral: e.target.value }))}
                  className="w-full border border-[rgba(17,17,17,0.2)] bg-canvas px-3 py-2 text-[13px] outline-none focus:border-ink" />
              </div>
            </div>
            ...
            <AnimatePresence>
              {verdict && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <p className="text-[10px] font-financial uppercase tracking-widest text-muted-ink mb-2">Generated Term Sheet (from Credit Review)</p>
                  <CreditTermSheet termSheet={verdict.termSheet} />
                </motion.div>
              )}
            </AnimatePresence>

            <Button onClick={handleSubmit} disabled={!form.poolId || !form.reviewId || !verdict}>
              Accept Terms & Submit Loan Request
            </Button>
          </div>
        ) : (
          <motion.div className="panel p-8 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <h3 className="font-heading font-bold text-[18px] text-ink mb-2">Loan Request Submitted</h3>
            <p className="text-[13px] text-muted-ink">Your loan has been recorded. Track repayment status in the Repayments page.</p>
            <Button variant="secondary" size="sm" className="mt-5" onClick={() => setSubmitted(false)}>
              Submit Another
            </Button>
          </motion.div>
        )}
